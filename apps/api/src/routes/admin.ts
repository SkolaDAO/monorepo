...
adminRouter.post("/test-referral", async (c) => {
  const { referralStats, purchases } = await import("../db/schema");
  
  // Get the admin user making the request
  const admin = c.get("user");
  
  // Get admin's current stats
  const statsBefore = await db.query.referralStats.findFirst({
    where: eq(referralStats.userId, admin.id),
  });
  
  // Simulate a referral earning
  const testEarning = "0.50";
  ...
  await db
    ...
    .values({
      userId: admin.id,
      totalReferrals: 1,
      totalEarningsUsd: testEarning,
    })
    .onConflictDoUpdate({
      target: referralStats.userId,
      set: {
        totalReferrals: sql`${referralStats.totalReferrals} + 1`,
        totalEarningsUsd: sql`${referralStats.totalEarningsUsd} + ${testEarning}`,
        updatedAt: new Date(),
      },
    });
  ...
  return c.json({
    success: true,
    before: {
      totalReferrals: statsBefore?.totalReferrals ?? 0,
      totalEarningsUsd: statsBefore?.totalEarningsUsd ?? "0",
    },
    after: {
      totalReferrals: statsAfter?.totalReferrals ?? 0,
      totalEarningsUsd: statsAfter?.totalEarningsUsd ?? "0",
    },
    message: `Referrals: ${statsBefore?.totalReferrals ?? 0} → ${statsAfter?.totalReferrals ?? 0}`,
  });
});
